...
<head>
	...
	<div class="col-md-offset-2 col-md-8 font-nbgl listdiv">
		<table class="table table-bordered table-striped table-hover">
			<thead>
				<tr>
					<th><a href="${pageContext.request.contextPath}/pc/hairart/list?page=1&sort=no
					...
					<th>제목</th>
					<th><a href="${pageContext.request.contextPath}/pc/hairart/list?page=1&sort=viewcount
								<c:if test='${not empty param.search}'>&search=${param.search}</c:if>
								<c:if test='${not empty param.keyword}'>&keyword=${param.keyword}</c:if>
					" class="sortable">조회수<c:if test="${param.sort=='viewcount'}"> <i class="fas fa-caret-down"></i></c:if></a></th>
					<th><a href="${pageContext.request.contextPath}/pc/hairart/list?page=1&sort=date_mod
								<c:if test='${not empty param.search}'>&search=${param.search}</c:if>
								<c:if test='${not empty param.keyword}'>&keyword=${param.keyword}</c:if>
			...
			<tbody>
				<c:forEach var="boardDto" items="${list}">
					<tr>
						...
						<td><a href="${pageContext.request.contextPath}/pc/hairart?no=${boardDto.no}
												<c:if test='${not empty param.page}'>&page=${param.page}</c:if>
												<c:if test='${not empty param.sort}'>&sort=${param.sort}</c:if>
												<c:if test='${not empty param.search}'>&search=${param.search}</c:if>
												<c:if test='${not empty param.keyword}'>&keyword=${param.keyword}</c:if>
						">${boardDto.title}<c:if test="${boardDto.files>0}"> <i class="fas fa-paperclip"></i></c:if></a></td>
		...
		<nav aria-label="Page navigation">
			<ul class="pagination">
				<c:choose>
					...
					<c:otherwise>
						<li>
							<a href="${pageContext.request.contextPath}/pc/hairart/list?page=${pagenationprev}
									<c:if test='${not empty param.sort}'>&sort=${param.sort}</c:if>
									<c:if test='${not empty param.search}'>&search=${param.search}</c:if>
									<c:if test='${not empty param.keyword}'>&keyword=${param.keyword}</c:if>
							" aria-label="Previous">
				...
				<c:forEach var="pagenation" begin="${pagenationmin}" end="${pagenationmax}" step="1">
					<li id="pagenation_${pagenation}">
						<a href="${pageContext.request.contextPath}/pc/hairart/list?page=${pagenation}
										<c:if test='${not empty param.sort}'>&sort=${param.sort}</c:if>
										<c:if test='${not empty param.search}'>&search=${param.search}</c:if>
										<c:if test='${not empty param.keyword}'>&keyword=${param.keyword}</c:if>
						">${pagenation}</a>
				...
				<c:choose>
					...
					<c:otherwise>
						<li>
							<a href="${pageContext.request.contextPath}/pc/hairart/list?page=${pagenationnext}
									<c:if test='${not empty param.sort}'>&sort=${param.sort}</c:if>
									<c:if test='${not empty param.search}'>&search=${param.search}</c:if>
									<c:if test='${not empty param.keyword}'>&keyword=${param.keyword}</c:if>
									" aria-label="Next">
	...
	<div class="col-md-offset-2 col-md-8 font-nbgl font-1.5rem text-center">
		<form action="${pageContext.request.contextPath}/pc/hairart/list" method="GET">
			...
			<div class="form-group form-inline">
				...
				<a href="${pageContext.request.contextPath}/pc/hairart/list" class="btn btn-primary">전체조회</a>
			</div>
		</form>
	</div>
