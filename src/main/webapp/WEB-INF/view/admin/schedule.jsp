...
/*  			border: red dotted 1px; */
	...
	<script>
		...
		window.addEventListener('resize', function() {
			if(totimetable) {
				forBtnCases();
				$('#helpth').css('width',$('.container').width());
				$('#helpth').css('left',$('#timetable').offset().left);
			}
		});
		...
		$(document).ready(function() {
			...
			forCalendarClick();
			if(settoday!='') {
				selecteddate = settoday.trim();
				afterCalendarClick();
			};
		});
		function firstButtonSet() {
			$('#mod_btn-ok').on('click',function(){
				if(!$('#mod_customer_name').val()) {
					alert('비회원이라면 이름을 적어주세요!');
					return;
				}
			...
			$('#mod_btn-force').on('click',function(){
				if(!$('#mod_customer_name').val()) {
					alert('비회원이라면 이름을 적어주세요!');
					return;
				}
			...
			$('#new_btn-ok').on('click',function(){
				...
				if(!$('#new_customer_gender').val()) {
					alert('비회원이라면 성별을 선택해주세요!');
					return;
				}
			...
			$('#new_btn-force').on('click',function(){
				if(!$('#new_customer_name').val()) {
					alert('비회원이라면 이름을 적어주세요!');
					return;
				}
		...
		function forSearchAjax() {
			if($('#search_name').val()!='' || $('#search_phone').val()!='') {
				$.ajax({
	                url: "schedule/ajax_search",
	                data:{
		...
		function afterSearch() {
			$('.tr_new').each(function(i, element){
				$(element).on('click', function() {
					$('.panel-warning').addClass('hidden');
					initMod();
					$('.panel-info').removeClass('hidden');
					trToNew($(this).attr('id'));
					$('html, body').animate({scrollTop: $('.panel-info').offset().top}, 500);	
				});
			});
		};
		...
		function forNewAjax() {
			$.ajax({
                ...
                data:{
                	...
                	customer_str:$('#new_customer').val(),
                	customer_name:$('#new_customer_name').val(),
                	customer_gender:$('#new_customer_gender').val(),
		...
		function forModAjax() {
			$.ajax({
                ...
                data:{
                	...
                	customer_str:$('#mod_customer').val(),
                	customer_name:$('#mod_customer_name').val(),
                	customer_gender:$('#mod_customer_gender').val(),
                	customer_phone:$('#mod_customer_phone').val(),
		...
		function trToNew(str) {
			var customer = str.substring(str.indexOf('_no')+3,str.indexOf('_name'));
			var name = str.substring(str.indexOf('_name')+5,str.indexOf('_gender'));
			var gender = str.substring(str.indexOf('_gender')+7,str.indexOf('_phone'));
			var phone = str.substring(str.indexOf('_phone')+6,str.indexOf('_designer'));
			var designer = str.substring(str.indexOf('_designer')+9,str.indexOf('_end'));
			initNew();
			$('#new_customer_fake').val('회원번호:'+customer);
			$('#new_customer').val(customer);
			$('#new_customer_name').val(name);
			if(gender=='남') {
				$('#new_customer_gender_m').prop('selected',true);
			} else {
				$('#new_customer_gender_f').prop('selected',true);
			}
			$('#new_customer_phone').val(phone);
			if(designer!=null) $('#new_designer_'+designer).prop('selected',true);
			$('#new_customer_name').prop('disabled',true);
			$('#new_customer_phone').prop('disabled',true);
			$('#new_customer_gender').prop('disabled',true);
		};
		...
		function forErrorCountAjax() {
			$.ajax({
                url: "schedule/ajax_errorcount",
                type:"post",
                success:function(response){
                	$('#num_error').replaceWith(response);
                	forBtnCases();
                }
            });
		};
		function forErrorPanelAjax() {
			$.ajax({
                url: "schedule/ajax_errorpanel",
                type:"post",
                success:function(response){
                	$('#notice').replaceWith(response);
                	forBtnCases();
                }
            });
		};
		function initMod() {
			$('#mod_no').val('');
			$('#mod_customer_fake').val('');
			$('#mod_customer').val(null);
			$('#mod_customer_name').val('');
			$('#mod_customer_phone').val('');
			$('.mod_customer_genders').prop('selected',false);
		...
		function initNew() {
			$('#new_customer_fake').val('검색으로 선택(혹은 비회원)');
			$('#new_customer').val(null);
			$('#new_customer_name').val('');
			$('#new_customer_phone').val('');
			$('.new_customer_genders').prop('selected',false);
			$('#new_customer_gender_none').prop('selected',true);
			// $('.new_designers').prop('selected',false);
		...
		function ajaxResultToMod(str) {
			initMod();
			var no = str.substring(str.indexOf('_no')+3,str.indexOf('_customer'));
			var customer = str.substring(str.indexOf('_customer')+9,str.indexOf('_name'));
			var name = str.substring(str.indexOf('_name')+5,str.indexOf('_gender'));
			var gender = str.substring(str.indexOf('_gender')+7,str.indexOf('_phone'));
			var phone = str.substring(str.indexOf('_phone')+6,str.indexOf('_designer'));
			...
			if(customer==0) {
				note = '비회원';
				$('#mod_customer_name').prop('disabled',false);
				$('#mod_customer_phone').prop('disabled',false);
				$('#mod_customer_gender').prop('disabled',false);
			} else {
				note = '회원번호:'+customer;
				$('#mod_customer').val(customer);
			}
			$('#mod_customer_fake').val(note);
			$('#mod_customer_name').val(name);
			if(phone=='null') $('#mod_customer_phone').val(null);
			else $('#mod_customer_phone').val(phone);
			if(gender=='남') {
				$('#mod_customer_gender_m').prop('selected',true);
			} else {
				$('#mod_customer_gender_f').prop('selected',true);
			}
...
<body>
	<div class="admin_body container">
		<div class="col-md-12 font-nbgb">
			<h1><i class="far fa-calendar-alt"></i> 스케쥴 관리</h1>
		</div>
		<div class="col-md-12 empty-row"></div>
		<div class="col-md-12" id="selecteddatediv">
		...
		<div class="col-md-12 empty-row"></div>
		<div class="col-md-12 font-1.5rem" id="notice">
			<c:if test="${num_error>0}">
				<div class="panel panel-danger">
					<div class="panel-heading">
						<h3 class="panel-title">${num_error}개의 수정사항이 있습니다!</h3>
					</div>
					<div class="panel-body">
						<c:forEach var="errorR" items="${errorlist}">
							<p>
								${errorR.whatday_str} ${errorR.customer_name}님의 ${errorR.starttime_str}~${errorR.endtime_str} 예약이 수정되어야 합니다.
							</p>
						</c:forEach>
					</div>
				</div>
		...
		<div class="col-md-12 font-2rem text-right fortimetable">
			<a href="schedule" class="btn btn-primary">날짜 선택으로 돌아가기</a>
		</div>
		<div class="col-md-12 empty-row"></div>
		<div class="col-md-12 font-2rem" id="calendar">
		...
		<div class="col-md-12 font-1.5rem hidden" id="helpth">
		...
		<div class="col-md-12 font-1.5rem fortimetable" id="timetable">
			<c:if test="${not empty designerlist}">
				<table class="table table-striped table-bordered table-hover table-condensed">
					<thead>
						<tr class="font-nbgb">
		...
		<div class="col-md-12 font-nbgl fortimetable hidden">
			시간표 안내 -  
			<button class="btn btn-warning cursordefault">예약된 항목</button>
			<button class="btn btn-success cursordefault">시술 중인 항목</button>
			<button class="btn btn-danger cursordefault">수정해야 하는 항목</button>
		</div>
		<div class="col-md-12 empty-row fortimetable hidden"></div>
		<div class="col-md-6 font-nbgl fortimetable hidden">
			<div class="panel panel-success">
				<div class="panel-heading">
					<h3 class="panel-title">회원검색</h3>
				</div>
				<div class="panel-body">
					...
					<div class="form-group">
						<label for="search_phone">연락처 일부분</label>
						<input type="number" class="form-control" name="search_phone" id="search_phone">
					</div>
					<div class="form-group text-right">
						<button class="btn btn-success" id="btn-search">검색</button>
					</div>
				</div>
			</div>
			<div class="" id="divforsearch">
			</div>
		</div>
		<div class="col-md-3 font-nbgl fortimetable hidden">
			<div class="panel panel-info hidden">
				...
				<div class="panel-body">
					<div class="form-group">
						<label for="new_customer_fake">회원</label>
						<input type="text" class="form-control" name="new_customer_fake" id="new_customer_fake" disabled value="검색으로 선택(혹은 비회원)">
						<input type="hidden" name="customer" id="new_customer">
					</div>
					<div class="form-group">
						<label for="new_customer_name">이름(비회원)</label>
						<input type="text" class="form-control" name="customer_name" id="new_customer_name" required disabled>
					</div>
					<div class="form-group">
						<label for="new_customer_gender">성별(비회원)</label>
						<select class="form-control" name="customer_gender" id="new_customer_gender" required disabled>
							<option value="X" class="new_customer_genders" id="new_customer_gender_none" disabled selected>- 선택 -</option>
							<option value="남" class="new_customer_genders" id="new_customer_gender_m">남</option>
							<option value="여" class="new_customer_genders" id="new_customer_gender_f">여</option>
						</select>
					</div>
					<div class="form-group">
						<label for="new_customer_phone">연락처(비회원)</label>
						<input type="text" class="form-control" name="customer_phone" id="new_customer_phone" disabled>
					</div>
					<div class="form-group">
					...
					<div class="form-group text-right">
						...
						<button class="btn btn-primary" id="new_btn-force">강제등록</button>
		...
		<div class="col-md-3 font-nbgl fortimetable hidden">
			<div class="panel panel-warning hidden">
				...
				<div class="panel-body">
					...
					<div class="form-group">
						<label for="mod_customer_fake">회원여부</label>
						<input type="text" class="form-control" name="mod_customer_fake" id="mod_customer_fake" disabled>
						<input type="hidden" name="customer" id="mod_customer">
					</div>
					<div class="form-group">
						<label for="mod_customer_name">이름</label>
						<input type="text" class="form-control" name="customer_name" id="mod_customer_name" required disabled>
					</div>
					<div class="form-group">
						<label for="mod_customer_gender">성별</label>
						<select class="form-control" name="customer_gender" id="mod_customer_gender" required disabled>
							<option value="남" class="mod_customer_genders" id="mod_customer_gender_m">남</option>
							<option value="여" class="mod_customer_genders" id="mod_customer_gender_f">여</option>
						</select>
					</div>
					<div class="form-group">
						<label for="mod_customer_phone">연락처</label>
						<input type="text" class="form-control" name="customer_phone" id="mod_customer_phone" disabled>
					</div>
					<div class="form-group">
					...
					<div class="form-group text-right">
						...
						<button class="btn btn-primary" id="mod_btn-force">강제수정</button>
